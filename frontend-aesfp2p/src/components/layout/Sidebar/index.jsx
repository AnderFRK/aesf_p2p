...
export default function Sidebar({ 
  ...
  return (
    <aside className="w-64 bg-gray-800 flex flex-col border-r border-gray-700 relative z-20 shrink-0">
        <div className="h-12 flex items-center px-4 font-bold border-b border-gray-700 shadow-sm text-emerald-400 hover:bg-gray-700/50 transition-colors cursor-pointer">
          AESF P2P Alpha
        </div>
        
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-6 custom-scrollbar">
          <div>
            <h3 className="px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1 hover:text-gray-400 cursor-pointer">
          ...
          <div>
            <div className="flex items-center justify-between px-2 mb-2 group">
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-1 hover:text-gray-400 cursor-pointer">
  ...
  if (isVoice) {
      return (
        <div className="mb-0.5">
          <div onClick={() => onJoinVoice(canal.id)} className={`group flex items-center px-2 py-1.5 rounded mx-1 transition-all duration-200 cursor-pointer ${isActive ? 'bg-gray-700/80 text-white shadow-sm' : 'text-gray-400 hover:bg-gray-700/50 hover:text-gray-200'}`}>
